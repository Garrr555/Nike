      ...
      <div className="bg-dark text-white p-5 w-[300px] h-[100vh] flex flex-col justify-between">
        <div>
          ...
          {lists.map((list, index) => (
            <Link
            href={list.url}
              className={`text-lg font-semibold my-3 flex items-center gap-2 rounded-lg p-1 transition-all duration-300 hover:bg-white hover:text-primary cursor-pointer ${pathname === list.url && 'bg-white text-primary'}`}
              key={list.title}
            >
